...
function CompareCard() {
  ...
  const series: LivelineSeries[] = useMemo(
    () => [
      { id: "mint", label: "", data: data.mint, value: latestMint, color: "#f68f3c" },
      {
        id: "pistachio",
        label: "",
        data: data.pistachio,
    ...
    <div className="min-h-[278px] rounded-card bg-surface p-3 shadow-hairline">
      ...
      <div className="mt-2 overflow-hidden rounded-control bg-inset shadow-hairline">
        ...
        <div
          ...
          onPointerDown={(event) =>
            setHoverIndex(chartIndexFromPointer(event, data.mint.length))
          }
          onPointerMove={(event) =>
            setHoverIndex(chartIndexFromPointer(event, data.mint.length))
          }
          ...
          {hoverIndex !== null && (
            <>
              <span
                ...
                style={{ left: `${(hoverIndex / (data.mint.length - 1)) * 100}%` }}
              />
              <span
                className="insight-chart-tooltip-anchor"
                style={{
                  left: `${Math.min(Math.max((hoverIndex / (data.mint.length - 1)) * 100, 28), 72)}%`,
                }}
              >
                <ChartTooltip
                  rows={[
                    {
                      label: "Mint Chip",
                      value: formatPercent(data.mint[hoverIndex].value),
                      color: "var(--orange)",
                    },
                    {
                      label: "Pistachio",
                      value: formatPercent(data.pistachio[hoverIndex].value),
                      color: "var(--blue)",
                    },
                  ]}
                />
    ...
    <div className="min-h-[278px] rounded-card bg-surface p-3 shadow-hairline">
      ...
      <div className="mt-3 min-h-16 rounded-control bg-inset px-2.5 py-2 shadow-hairline">
        ...
        <span className="mt-1 block text-[11px] leading-relaxed text-ink-3">
          Contribution snapshot across current inventory value. Segment selection
          changes the inspected group without moving the card.
        </span>
      </div>
    </div>
  );
...
const PAGES = [
  {
    ...
    prose: (
      <>
        ...
        Rocky Road — down <Mono tone="red">-6%</Mono> or{" "}
  ...
  },
  {
    ...
    prose: (
      <>
        ...
        <span className="font-medium text-ink">Dec 13</span> —{" "}
  ...
  },
  {
    ...
    prose: (
      <>
        ...
        it&apos;s <span className="font-medium text-ink">72.5%</span> of your case.
      </>
    ),
    Card: AllocationCard,
...
export function InsightCards() {
  ...
  return (
    <div className="w-full max-w-86">
      ...
      {/* page content */}
      <div key={PAGES[page].key} style={{ animation: "fade-in 250ms ease-out both" }}>
        <p className="mt-1.5 text-[12.5px] leading-relaxed text-ink-2">{prose}</p>
        <div className="mt-2">
          <Card />
        </div>
        <button className="mt-2 rounded-full bg-surface px-3 py-1.5 text-left text-[12px] text-ink shadow-btn transition-colors duration-100 hover:bg-hover">
          {pill}
        </button>
      </div>
    </div>
