...
const OPTIONS: Option[] = [
  ...
  },
  {
    ...
    body: (
      <>
        Fall back to a <span className="font-medium text-ink">full restock</span>{" "}
        across every SKU.
      </>
    ),
      ...
      <div className="primitive-card-footer flex items-center justify-between gap-3 border-t border-line bg-inset">
        ...
        <span className="flex items-center gap-2">
          <button
            ...
            className={`h-7 rounded-control px-2.5 text-[12.5px] font-medium shadow-btn transition-[background-color,transform] duration-100 active:scale-[0.96] ${open ? "bg-hover text-ink" : "bg-surface text-ink hover:bg-hover"}`}
          >
            Alternatives
          </button>
          <button
            ...
            className={`h-7 rounded-control px-3 text-[12.5px] font-medium shadow-[inset_0_1px_0_rgba(255,255,255,0.14),0_0_0_1px_rgba(16,24,40,0.12),0_1px_2px_rgba(16,24,40,0.1)] transition-[background-color,transform] duration-150 active:scale-[0.96] ${accepted ? "bg-green text-white" : active.ctaStyle}`}
          >
